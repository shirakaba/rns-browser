...
var WKNavigationDelegateImpl = (function (_super) {
    ...
    // JB
    WKNavigationDelegateImpl.prototype.webViewDidCommitNavigation = function (webView, navigation) {
        if (web_view_common_1.traceEnabled()) {
            web_view_common_1.traceWrite("WKNavigationDelegateClass.webViewDidCommitNavigation(" + webView.URL + ")", web_view_common_1.traceCategories.Debug);
        }
        var owner = this._owner.get();
        if (owner) {
            var src = owner.src;
